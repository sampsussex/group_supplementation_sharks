#!/usr/bin/env python
"""Auto-converted from GAMA_supplementation_HMF.ipynb."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="GAMA supplementation HMF pipeline.")
    parser.add_argument(
        "--gama-galaxies-fits",
...
# Calculating the HMF for the supplemented luminosity mass (mass_sup)
# This is done using np.histogram, with bins being weighted by 1/vmax
# At this point, errors are just poisson

bins = np.linspace(11.2,15.4,22)

Nbin, edges = np.histogram(np.log10(sample['mass_sup']), bins=bins)
diff=np.diff(edges)[0]
phi_total_lum, edges = np.histogram(np.log10(sample['mass_sup']), bins=bins, weights=1.0/(diff*sample['Vmax']))
Mbin = edges[:-1] + np.diff(edges)
phi_err_poisson = phi_total_lum/Nbin**0.5

# MRP fitting function for HMF
...
# Creating a function that takes 1000 samples from the luminosity-mass scaling relation using its errors
def massconv_sample(lum):
    return 1e14 * h**-1 * np.random.normal(0.81,0.04,1000) * (lum/(10**11.5 ))**np.random.normal(1.01,0.07,1000)

# These samples are inputted into a dataframe, where each group is given 1000 independent mass estimates
dfHalo_sample=pd.DataFrame(columns=np.linspace(0,999,1000).astype(int), index=dfHalo.index)

for halo in tqdm(dfHalo.index):
    lum=dfHalo.loc[halo]['lum_sup']#*dfHalo.loc[halo]['Lumfac21']
    dfHalo_sample.loc[halo]=massconv_sample(lum).astype(float)
...
# Defining our log likelihood, priors and posterior
...
# Removing the first 4 bins from fitting, as they outside the completeness window
Mbin_new=Mbin_new[4:]
phi_means=phi_means[4:]
err_new=err_new[4:]
...
plt.savefig(plots_dir / "HMF_corner.jpg",bbox_inches='tight',dpi=200)
...
# Returning the parameters as those with the maximum likelihood
...
# Plotting histogram at top
Nbin,_=np.histogram(np.log10(sample['mass_sup']), bins=bins)
edges = []
for M in Mbin:
...
axs[0].set_title('Supplemented Luminosity mass, '+'$N_\mathrm{FoF} \geq 5$')



# Plotting Driver and lcdm
...
# Plotting our HMF with errors bars
...
# Sampling parameters to plot the fits
...
axs[1].axvspan(10**11, 10**12.9 , color='k', alpha=0.1, lw=1)
...
plt.savefig(plots_dir / "HMF.jpg",bbox_inches='tight',dpi=200)
